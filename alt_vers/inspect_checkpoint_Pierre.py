        ...
        if all_tensors:
            ...
            for key in sorted(var_to_shape_map):
                if not any([e in key for e in bad_keys]):
                    ...
                    # pierre_save_matrices(tensor_name, reader)
            # print('filters, biases lengths: ', len(filters), len(biases))
...
def pierre_save_matrices(tensor_name, reader):
    """added by Pierre to save matrices into a file"""
    delimiter = ' '
    newline = ' ] ; ... \n [ '
    fmt = '%1.8e'
    tensor_name_out = tensor_name.replace('/', '_')
    np.savetxt(tensor_name_out, reader.get_tensor(tensor_name),
               delimiter=delimiter, newline=newline, fmt=fmt)


# As far as I can tell, this currently isn't used for anything
def parse_numpy_printoption(kv_str):
    """
    :param kv_str: A string of the form 'x=y', such as 'threshold=100000'
    :raises: argparse.ArgumentTypeError: If the string couldn't be used to set
    any nump printoption.
    :return:
    ...
    if len(k_v_str) != 2 or not k_v_str[0]:
        raise argparse.ArgumentTypeError(
            "'%s' is not in the form k=v." % kv_str)
    k, v_str = k_v_str
    printoptions = np.get_printoptions()
    if k not in printoptions:
        raise argparse.ArgumentTypeError(
            "'%s' is not a valid printoption." % k)
    v_type = type(printoptions[k])
    if v_type.isinstance(None):
        raise argparse.ArgumentTypeError(
    ...
    try:
        if v_type is not bool:
            ...
            v = flags.BooleanParser().parse(v_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Checkpoint filename. Note, if using Checkpoint V2 format, "
             "file_name is the shared prefix between all files in the "
             "checkpoint.")
    parser.add_argument(
        "--tensor_name",
        type=str,
